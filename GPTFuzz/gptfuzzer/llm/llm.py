...
# Keep the upstream module importable for API-only runs.  FastChat/vLLM and the
# provider SDKs are only needed when their corresponding backend is selected.
try:
    from fastchat.model import load_model, get_conversation_template
except ImportError:  # pragma: no cover - exercised only in API-only installs
    load_model = None
...
except ImportError:  # pragma: no cover - exercised only without Gemini support
    genai = None
    types = None

try:
    from anthropic import Anthropic
except ImportError:  # pragma: no cover - exercised only without Claude support
    Anthropic = None
...
class LocalLLM(LLM):
    ...
    def create_model(self, model_path,
                     ...
                     revision=None,
                     debug=False):
        if load_model is None or get_conversation_template is None:
            raise ImportError(
                "FastChat is required for LocalLLM. Install it only when using "
                "the upstream local Transformers backend."
            )
...
class LocalVLLM(LLM):
    def __init__(self,
        ...
        if vllm is None or SamplingParams is None:
            raise ImportError(
                "vLLM is required for LocalVLLM. Install it only when using "
                "the upstream local vLLM backend."
            )
        self.model_path = model_path
        self.model = vllm(
...
class ClaudeLLM(LLM):
    def __init__(self,
        ...
        if Anthropic is None:
            raise ImportError("anthropic is required for ClaudeLLM.")
